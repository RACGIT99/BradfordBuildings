	...
	gui.showQueryResult = function (point, layer, obj, show_coords) {
		...
		if (e) {
			...
			if (layer && layer.properties.propertyNames !== undefined) {
				...
				var stlength;
                var urlstring;
				stlength=rac1.length;
				stlength=stlength-5;
                stindex = rac1.search("http");
                stindex2 = rac1.search("uk");
				stindex2=stindex2+4;
                if (stindex != -1) {
                   urlstring = rac1.substring(stindex,stlength);
                   rac1=urlstring;
               };
...
e = E("qr_attrs_table");
		if (e) {
			...
			if (layer && layer.properties.propertyNames !== undefined) {
				var row;
				for (var i = 0, l = layer.properties.propertyNames.length-1; i < l; i++) {
					row = document.createElement("tr");
					row.innerHTML = "<td>" + layer.properties.propertyNames[i] + "</td>" +
									"<td>" + obj.userData.properties[i] + "</td>";
					e.appendChild(row);
...
///BELOW WORKS BUT WINDOW NOT CHANGED
///ON_CLICK("measurebtn2",function () { window.open(rac1),"", "width=100, height=100"});
//ON_CLICK("measurebtn2",<a href="https://www.bradfordonavonmuseum.co.uk/trinity" target="_blank" style="color: red;">Link Text</a>;
ON_CLICK("measurebtn2",function () { window.open(rac1)});	  
// narrative box
ON_CLICK("nextbtn", function () {
	app.animation.keyframes.resume();
});
///////////////
//<a href="https://www.bradfordonavonmuseum.co.uk/trinity" target="_blank" style="color: red;">Link Text</a>
